...
def _write_config(content: str, tmp_path: Path) -> str:
...
class TestConfigLoader:
    def test_loads_minimal_config(self, tmp_path: Path) -> None:
        podcast, config = load_config(_write_config(MINIMAL_CONFIG, tmp_path))
        assert podcast.name == "Test Show"
        assert podcast.target_length_minutes == 5
        assert podcast.show_prompt == ""

    def test_loads_show_prompt(self, tmp_path: Path) -> None:
        podcast, _ = load_config(_write_config(CONFIG_WITH_SHOW_PROMPT, tmp_path))
        assert podcast.show_prompt == "Be sarcastic and skip the fluff."

    def test_loads_web_page_source(self, tmp_path: Path) -> None:
        podcast, _ = load_config(_write_config(CONFIG_WITH_WEB_SOURCE, tmp_path))
        assert len(podcast.sources) == 2
        assert podcast.sources[0].kind.value == "web_page"
        assert podcast.sources[1].kind.value == "rss"

    def test_deterministic_podcast_id(self, tmp_path: Path) -> None:
        p1, _ = load_config(_write_config(MINIMAL_CONFIG, tmp_path))
        p2, _ = load_config(_write_config(MINIMAL_CONFIG, tmp_path))
        assert p1.id == p2.id
